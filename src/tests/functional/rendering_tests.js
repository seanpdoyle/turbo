...
test("triggers before-render and render events", async ({ page }) => {
  ...
  expect(await newBody).toEqual(await page.evaluate(() => document.body.outerHTML))
})
...
test("includes isPreview in render event details", async ({ page }) => {
  ...
  await nextEventNamed(page, "turbo:render")
  expect(await isPreview).toEqual(false)
})
...
test("reloads when tracked elements change", async ({ page }) => {
  ...
  expect(pathname(page.url())).toEqual("/src/tests/fixtures/tracked_asset_change.html")
...
test("reloads when tracked elements change due to failed form submission", async ({ page }) => {
  ...
  expect(pathname(page.url())).toEqual("/src/tests/fixtures/rendering.html")
  expect(await visitAction(page)).toEqual("load")
  expect(reason).toEqual("tracked_element_mismatch")
...
test("before-render event supports custom render function", async ({ page }) => {
  ...
  await expect(page.locator("#custom-rendered")).toHaveText("Custom Rendered")
})
...
test("before-render event supports async custom render function", async ({ page }) => {
  ...
  expect(renderedElement).toEqual("Custom Rendered")
})
...
test("wont reload when tracked elements has a nonce", async ({ page }) => {
  ...
  expect(pathname(page.url())).toEqual("/src/tests/fixtures/tracked_nonce_tag.html")
...
test("reloads when turbo-visit-control setting is reload", async ({ page }) => {
  ...
  expect(pathname(page.url())).toEqual("/src/tests/fixtures/visit_control_reload.html")
  expect(await visitAction(page)).toEqual("load")
...
test("maintains scroll position before visit when turbo-visit-control setting is reload", async ({ page }) => {
  ...
  expect(await isScrolledToTop(page)).toEqual(false)
  ...
  await page.click("#below-the-fold-visit-control-reload-link")
...
test("replaces provisional elements in head", async ({ page }) => {
  ...
  expect(await page.locator("meta[name=test]").count()).toEqual(0)

  await page.click("#same-origin-link")
  await nextBody(page)
  const newElements = await provisionalElements()
  expect(await deepElementsEqual(page, newElements, originalElements)).toEqual(false)
  expect(await page.locator("meta[name=test]").count()).toEqual(1)
...
test("restores focus during page rendering when transposing the activeElement", async ({ page }) => {
  ...
  await expect(page.locator("#permanent-input")).toBeFocused()
})
...
test("restores focus during page rendering when transposing an ancestor of the activeElement", async ({
  ...
  await expect(page.locator("#permanent-descendant-input")).toBeFocused()
})
...
test("before-frame-render event supports custom render function within turbo-frames", async ({ page }) => {
  ...
  await expect(customRendered).toHaveText("Custom Rendered Frame")
})
...
test("restores focus during turbo-frame rendering when transposing the activeElement", async ({ page }) => {
  ...
  await expect(page.locator("#permanent-input-in-frame")).toBeFocused()
})
...
test("restores focus during turbo-frame rendering when transposing a descendant of the activeElement", async ({
  ...
  await expect(page.locator("#permanent-descendant-input-in-frame")).toBeFocused()
})
...
test("preserves permanent element video playback", async ({ page }) => {
  ...
  expect(timeBeforeRender).not.toEqual(0)

  await page.click("#permanent-element-link")
  await nextEventNamed(page, "turbo:load")

  const timeAfterRender = await videoElement.evaluate((video) => video.currentTime)
  expect(timeAfterRender).toEqual(timeBeforeRender)
})
...
test("preserves input values", async ({ page }) => {
  ...
  expect(await propertyForSelector(page, "#text-input", "value")).toEqual("test")
  expect(await propertyForSelector(page, "#checkbox-input", "checked")).toEqual(true)
  expect(await propertyForSelector(page, "#radio-input", "checked")).toEqual(true)
  expect(await propertyForSelector(page, "#textarea", "value")).toEqual("test")
  expect(await propertyForSelector(page, "#select", "value")).toEqual("2")
  expect(await propertyForSelector(page, "#select-multiple", "value")).toEqual("2")
})
...
test("does not preserve password values", async ({ page }) => {
  ...
  expect(await propertyForSelector(page, "#password-input", "value")).toEqual("")
})
...
test("<input type='reset'> clears values when restored from cache", async ({ page }) => {
  ...
  expect(await propertyForSelector(page, "#text-input", "value")).toEqual("")
  expect(await propertyForSelector(page, "#checkbox-input", "checked")).toEqual(false)
  expect(await propertyForSelector(page, "#radio-input", "checked")).toEqual(false)
  expect(await propertyForSelector(page, "#textarea", "value")).toEqual("")
  expect(await propertyForSelector(page, "#select", "value")).toEqual("1")
  expect(await propertyForSelector(page, "#select-multiple", "value")).toEqual("")
})
...
test("rendering a redirect response replaces the body once and only once", async ({ page }) => {
  ...
  expect(await noNextBodyMutation(page)).toEqual(true)
})
